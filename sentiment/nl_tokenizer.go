...
func NewNLTokenizer() Tokenizer {
	return &nlTokenizer{}
}

func (t *nlTokenizer) Tokenize(text string) (words map[string]int64) {
	words = make(map[string]int64)
	for _, w := range splitTokenRxNL.Split(text, -1) {
		if len(w) > 2 {
